...
public:
	...
	*/
	...
	void colar(int pringao, Queue<T>& colegas) {
		if (!colegas.empty()) {
			Nodo *_act = _prim;
			while (_act != nullptr && _act->_elem != pringao) {
				_act = _act->_sig;
			}
			if (_act != nullptr) {
				if (_act == _ult) _ult = colegas._ult;
				Nodo *aux = _act->_sig;
				_act->_sig = colegas._prim;
				colegas._ult->_sig = aux;
				_numElems += colegas._numElems;

			}
			colegas._prim = nullptr;
			colegas._ult = nullptr;
			colegas._numElems = 0;
