...
class Set {
	...
	public:
		Set(int initCapacity = DEFAULT_CAPACITY) : 	size(0), capacity(initCapacity), elems(new T[capacity]) {};
		Set(const Set<T>& set);
		~Set();
		bool isEmpty() const;
		void add(const T& x);
...
template<class T>
Set<T>::Set(const Set<T>& set) : size(set.size), capacity(set.capacity), elems(new T[capacity]) {
